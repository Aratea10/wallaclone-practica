...
function renderAd(ad) {
    ...
    if (ad.photo) {
        imageContainer.innerHTML = `<img src="http://127.0.0.1:8000${ad.photo}" alt="${ad.name}" class="w-full h-96 object-cover" />`;
    } else {
...
async function loadAd() {
    ...
    try {
        const ad = await client.get(`/api/adverts/${adId}`);
        renderAd(ad);
        showState("success");
    } catch (error) {
